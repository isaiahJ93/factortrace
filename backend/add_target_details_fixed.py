...
for i, line in enumerate(lines):
    if 'base_div.tail = " tCO₂e"' in line:
        ...
        # Build the code without f-string issues
        target_details = f'''
{spaces}
...
{spaces}type_elem = ET.SubElement(p_type, namespaces["ix"] + 'nonNumeric', attrib={{
...
{spaces}year_elem = ET.SubElement(p_year, namespaces["ix"] + 'nonFraction', attrib={{
...
{spaces}reduction_elem = ET.SubElement(p_reduction, namespaces["ix"] + 'nonFraction', attrib={{
...
{spaces}netzero_elem = ET.SubElement(p_netzero, namespaces["ix"] + 'nonFraction', attrib={{
