...
# Find where we close the body and add contexts just before
# Look for where we create the tree and return
pattern = r'(\s+)(tree = ET\.ElementTree\(html\))'

insertion = r'''\1# Add XBRL contexts and units in body before closing
...
\1id4.text = data.get('lei', '529900HNOAA1KXQJUQ27')
\1period4 = ET.SubElement(ctx4, f'{{{namespaces["xbrli"]}}}period')
\1instant4 = ET.SubElement(period4, f'{{{namespaces["xbrli"]}}}instant')
\1instant4.text = f"{data.get('targets', {}).get('base_year', data.get('reporting_period', 2025))}-12-31"
\1
\1# Units
...
print("✅ Added XBRL contexts and units to BODY before closing!")
