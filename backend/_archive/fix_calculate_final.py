...
# Create a clean calculate function
clean_calculate = '''@router.post("/calculate")
def calculate_emission(request_body: dict, db: Session = Depends(get_db)):
    ...
    # Get all emission factors to find the right one
    all_factors = [
        # Scope 1
        ...
        {"id": 20, "name": "Electronics", "factor": 24.0, "unit": "kg"},
...
# Look for @router.post("/calculate") and replace everything until the next @ or end of file
pattern = r'@router\.post\("/calculate"\).*?(?=@router|$)'
content = re.sub(pattern, clean_calculate, content, flags=re.DOTALL)
