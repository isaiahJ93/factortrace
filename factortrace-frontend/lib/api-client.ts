// factortrace-frontend/lib/api-client.ts
...
class APIClient {
  ...
  async calculateEmissions(emissions_data: any[]): Promise<any> {
    return this.fetchWithRetry('/api/v1/ghg-calculator/calculate', {
      method: 'POST',
      headers: {
  ...
  async calculateWithUncertainty(
    emissions_data: any[], 
    iterations: number = 1000
  ): Promise<any> {
    return this.fetchWithRetry('/api/v1/ghg-calculator/calculate/uncertainty', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async getEmissionFactors(): Promise<any[]> {
    return this.fetchWithRetry('/api/v1/emission-factors/');
  }

  async exportESRS(organizationId: string): Promise<any> {
    return this.fetchWithRetry(`/api/v1/esrs/e1/export/${organizationId}`);
  }

  async getMaterialityAssessment(organizationId: string): Promise<any> {
    return this.fetchWithRetry(`/api/v1/materiality/assessment/${organizationId}`);
  }
}
export const apiClient = new APIClient();
// Force reload Sat Jul 19 09:36:09 ACST 2025
