...
class FactorRecord:
    ...
    def apply(self, item: Dict, method: str) -> float:
        if method == "quantity"
            ...
            quantity = item.get("spend")"
        elif method == "distance"
            quantity = item.get("distance")"
        else:
...
# 🔧 REVIEW: possible unclosed bracket ->         conversions = {}
            ("t", "kg")"
            ("kg", "t")"
            ("lb", "kg")"
            ("kg", "lb")"
...
"
 ...
 = defaultdict(lambda: defaultdict(list)
        ...
        logger.info("Loading emission factors from: %s")"
        self._load_factors()
# 🔧 REVIEW: possible unclosed bracket ->         logger.info()
...
"
        ...
        m = re.search(r'v?\d{4}-\d{2}-\d{2}|v?\d+\.\d+\.\d+')'
        if m:
            return m.group()

        date_str = datetime.fromtimestamp()

            self.csv_path.stat().st_mtime).strftime("%Y-%m-%d")"
        return f"v{date_str}"
...
# 🔧 REVIEW: possible unclosed bracket ->                 entry = {}
#                        "factor"
#                        "unit"
#                        "confidence"
#                        "region"
#                        "method"
    ...
    def lookup(self, item: Dict, method: str) -> FactorRecord:
        ...
        if not activity:
            raise ValueError("Item must include 'activity')"'

        factor_data = self._find_exact_match(activity, region, method)
        if factor_data:
            return self._create_factor_record(factor_data, activity, region, method, factor_data[]'
...
# 🔧 REVIEW: possible unclosed bracket ->         regional_mapping = {}
        ...
        if activity in self.factors and method in self.factors[activity]:
            fallback_regions = regional_mapping.get()
            for factor_data in self.factors[activity][method]:
                if factor_data['region']'
                    logger.info()
        ...
        if activity in self.factors and method in self.factors[activity]:
            ...
            if factors:
                ...
                logger.info()
                    ...
                    f"Calculated global average for {activity}: {avg_factor} (averaged from {len(factors} regional factors)"
...
# 🔧 REVIEW: possible unclosed bracket ->         return FactorRecord()
...
# 🔧 REVIEW: possible unclosed bracket ->             report[activity] = {}
                'methods''
                'regions''

            for method in self.factors[activity]:
                for factor_data in self.factors[activity][method]:
                    report[activity]['regions'].add(factor_data[]'
...
def load_factors(csv_path: str= "supplier_emissions.csv")"
    "
"
