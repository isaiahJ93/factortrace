#!/bin/bash
# Auto-tag ESRS Climate Disclosure Report with EFRAG/CSRD concepts
...
# Function to extract numeric values with units
extract_value() {
    echo "$1" | grep -oE '[0-9]{1,3}(,[0-9]{3})*(\.[0-9]+)?' | head -1
}

# Function to extract monetary values
extract_money() {
    echo "$1" | sed 's/EUR //g' | grep -oE '[0-9]+(\.[0-9]+)?' | head -1
}
...
# Calculate and add total emissions
scope1=$(grep "GHG Emissions Scope 1:" "$INPUT_FILE" | extract_value)
scope2=$(grep "GHG Emissions Scope 2:" "$INPUT_FILE" | extract_value)
scope3=$(grep "GHG Emissions Scope 3 - Total:" "$INPUT_FILE" | extract_value)
