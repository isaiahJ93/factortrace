#!/bin/bash
# Auto-tag document with EFRAG/CSRD concepts - Fixed version
...
# Improved function to extract numeric values including comma-separated thousands
extract_value() {
    # Extract numbers with commas, decimals, and percentage signs
    echo "$1" | grep -oE '[0-9]{1,3}(,[0-9]{3})*(\.[0-9]+)?%?|[0-9]+(\.[0-9]+)?%?' | head -1
}
