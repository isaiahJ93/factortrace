...
def generate_voucher(supplier_id: str) -> EmissionVoucher:
    """
    Stub generator — Replace with real logic later.
    """
...
class Scope3CategoryEnum(str, Enum):
    ...
    CAT_1_PURCHASED_GOODS = "category_1_purchased_goods_services"
...
class GWPVersionEnum(str, Enum):
    """IPCC Assessment Report GWP versions per ESRS E1-6 §AR6"""

    AR4_100 = "AR4_100"  # Legacy
    AR5_100 = "AR5_100"  # Current transition
    AR6_100 = "AR6_100"  # CSRD mandatory from 2025
    AR6_20 = "AR6_20"  # Short-term impact assessment
...
class ConsolidationMethodEnum(str, Enum):
    """GHG Protocol consolidation approaches per ESRS E1-6 §67"""
...
class EmissionsRecord(BaseModel):
    """Emissions record per scope/category per ESRS E1-6 §44-53"""
    ...
    installation_id: Optional[str] = Field(None, description="CBAM installation ID")

    @field_validator("scope3_category")
    def validate_scope3_category(cls, v, info):
        """Ensure Scope 3 has category"""
        if info.data.get("scope") == ScopeLevelEnum.SCOPE_3 and v is None:
            raise ValueError("Scope 3 emissions must specify category per ESRS E1-6 §48")
        return v
