...
'''
    ...
    # Now find where to insert the methods
    # Look for the last method in the class (before line 1580 which is class end)
    class_end_line = None
    for i in range(1579, 1500, -1):  # Search backwards from class end
        if lines[i].strip().startswith('}') and 'private' in lines[i-10:i].join(''):
            class_end_line = i
            break
    
    if not class_end_line:
        # Alternative: find the addFooters method and insert after it
    ...
    if not class_end_line:
        class_end_line = 1579  # Use the known class end
...
'''
    ...
    # Insert methods before the class closing brace (line 1580)
    lines.insert(1579, new_methods)
